<?php
	...
	//1. hago insert a la tabla de logs
        $where_log=str_replace("'","\"",$where);
        $sql="insert into log_reportes (\"user\",\"parameters\") values ('$user','$fechaIni 00:00:00,$fechaFin 23:59:59,$where_log')";

        pg_query($sql);

	//2. hago la consulta
	$sql="select a.pedido,a.id_tecnico,(select nombre from tecnicos where identificacion=a.id_tecnico),(select ciudad from tecnicos where identificacion=a.id_tecnico) as ciudad,a.empresa,a.asesor,a.observaciones,a.accion,a.tipo_pendiente,to_char(a.fecha,'yyyy-mm-dd hh24:mi:ss') as fecha2, a.proceso, a.producto,a.duracion,a.llamada_id, a.prueba_integrada,a.codigo_familiar,a.smartplay,a.toip,a.inter,a.iptv,a.telev,a.totdm from registros  a where a.fecha between '$fechaIni 00:00:00' and '$fechaFin 23:59:59' $where";
...
<tr> 
	...
	<select id="campo" > 
		...
		<option value='proceso' <? if($campo=="proceso") echo "selected"; ?> >Proceso</option>
		<option value='producto' <? if($campo=="producto") echo "selected"; ?> >Producto</option>
	</select> 
...
$sql="select a.pedido,b.nombre,a.accion,a.asesor,to_char(a.fecha,'yyyy-mm-dd hh24:mi:ss'),a.observaciones,a.id,a.duracion,a.fecha,b.ciudad,a.proceso from registros a,tecnicos b where b.identificacion=a.id_tecnico and a.fecha between '$fechaIni 00:00:00' and '$fechaFin 23:59:59' $where  order by a.fecha DESC limit $registrosPagina offset $offset";
